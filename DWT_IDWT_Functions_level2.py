...
class DWTFunction_2D(Function):
    @staticmethod
    def forward(ctx, input, matrix_Low_0, matrix_Low_1, matrix_High_0, matrix_High_1, L_lif, H_lif):
        ctx.save_for_backward(matrix_Low_0, matrix_Low_1, matrix_High_0, matrix_High_1)
        L = torch.matmul(matrix_Low_0, input)
        H = torch.matmul(matrix_High_0, input)
        L = L_lif(L.reshape(4, L.shape[0]//4, L.shape[1], L.shape[2], L.shape[3])).flatten(0, 1)
        H = H_lif(H.reshape(4, L.shape[0]//4, H.shape[1], H.shape[2], H.shape[3])).flatten(0, 1)
        LL = torch.matmul(L, matrix_Low_1).to(torch.float32)
        LH = torch.matmul(L, matrix_High_1).to(torch.float32)
        HL = torch.matmul(H, matrix_Low_1).to(torch.float32)
        HH = torch.matmul(H, matrix_High_1).to(torch.float32)
        return LL, LH, HL, HH
    @staticmethod
...
class IDWTFunction_2D(Function):
    ...
    def forward(ctx, input_LL, input_LH, input_HL, input_HH,
                matrix_Low_0, matrix_Low_1, matrix_High_0, matrix_High_1, iL_lif, iH_lif):
        ctx.save_for_backward(matrix_Low_0, matrix_Low_1, matrix_High_0, matrix_High_1)
        ...
        L = iL_lif(L.reshape(4, L.shape[0]//4, L.shape[1], L.shape[2], L.shape[3])).flatten(0, 1)
        H = iH_lif(H.reshape(4, L.shape[0]//4, H.shape[1], H.shape[2], H.shape[3])).flatten(0, 1)
        output = torch.add(torch.matmul(matrix_Low_0.t(), L), torch.matmul(matrix_High_0.t(), H)).to(torch.float32)
        return output
    @staticmethod
    def backward(ctx, grad_output):
        matrix_Low_0, matrix_Low_1, matrix_High_0, matrix_High_1 = ctx.saved_variables
        ...
        return grad_LL, grad_LH, grad_HL, grad_HH, None, None, None, None, None, None
...
class DWTFunction_3D(Function):
    ...
    def forward(ctx, input,
                ...
                L_lif, H_lif, LL_lif, LH_lif, HL_lif, HH_lif):
        ctx.save_for_backward(matrix_Low_0, matrix_Low_1, matrix_Low_2,
                              matrix_High_0, matrix_High_1, matrix_High_2)
        L = torch.matmul(matrix_Low_0, input)
        H = torch.matmul(matrix_High_0, input)
        L = L_lif(L.permute(2, 0, 1, 3, 4)).permute(1, 2, 0, 3, 4)
        H = H_lif(H.permute(2, 0, 1, 3, 4)).permute(1, 2, 0, 3, 4)
        LL = torch.matmul(L, matrix_Low_1)
        ...
        LL = LL_lif(LL.permute(2, 0, 1, 3, 4)).permute(1, 2, 3, 0, 4)
        LH = LH_lif(LH.permute(2, 0, 1, 3, 4)).permute(1, 2, 3, 0, 4)
        HL = HL_lif(HL.permute(2, 0, 1, 3, 4)).permute(1, 2, 3, 0, 4)
        HH = HH_lif(HH.permute(2, 0, 1, 3, 4)).permute(1, 2, 3, 0, 4)
        LLL = torch.matmul(matrix_Low_2, LL).transpose(dim0 = 2, dim1 = 3).to(torch.float32)
        LLH = torch.matmul(matrix_Low_2, LH).transpose(dim0 = 2, dim1 = 3).to(torch.float32)
        LHL = torch.matmul(matrix_Low_2, HL).transpose(dim0 = 2, dim1 = 3).to(torch.float32)
        LHH = torch.matmul(matrix_Low_2, HH).transpose(dim0 = 2, dim1 = 3).to(torch.float32)
        HLL = torch.matmul(matrix_High_2, LL).transpose(dim0 = 2, dim1 = 3).to(torch.float32)
        HLH = torch.matmul(matrix_High_2, LH).transpose(dim0 = 2, dim1 = 3).to(torch.float32)
        HHL = torch.matmul(matrix_High_2, HL).transpose(dim0 = 2, dim1 = 3).to(torch.float32)
        HHH = torch.matmul(matrix_High_2, HH).transpose(dim0 = 2, dim1 = 3).to(torch.float32)
...
class IDWTFunction_3D(Function):
    ...
    def forward(ctx, input_LLL, input_LLH, input_LHL, input_LHH,
                     ...
                     matrix_Low_0, matrix_Low_1, matrix_Low_2,
                     matrix_High_0, matrix_High_1, matrix_High_2,
                     iLL_lif, iLH_lif, iHL_lif, iHH_lif, iL_lif, iH_lif):
        ctx.save_for_backward(matrix_Low_0, matrix_Low_1, matrix_Low_2,
                              matrix_High_0, matrix_High_1, matrix_High_2)
        ...
        input_LL = iLL_lif(input_LL.permute(2, 0, 1, 3, 4)).permute(1, 2, 0, 3, 4)
        input_LH = iLH_lif(input_LH.permute(2, 0, 1, 3, 4)).permute(1, 2, 0, 3, 4)
        input_HL = iHL_lif(input_HL.permute(2, 0, 1, 3, 4)).permute(1, 2, 0, 3, 4)
        input_HH = iHH_lif(input_HH.permute(2, 0, 1, 3, 4)).permute(1, 2, 0, 3, 4)
        ...
        input_L = iL_lif(input_L.permute(2, 0, 1, 3, 4)).permute(1, 2, 0, 3, 4)
        input_H = iH_lif(input_H.permute(2, 0, 1, 3, 4)).permute(1, 2, 0, 3, 4)
    ...
    def backward(ctx, grad_output):
        ...
        return grad_LLL, grad_LLH, grad_LHL, grad_LHH, grad_HLL, grad_HLH, grad_HHL, grad_HHH, None, None, None, None, None, None, None, None, None, None, None, None
