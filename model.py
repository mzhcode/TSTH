...
class testTokenizer(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=512):
        ...
        self.dt3 = testSelfAttention(384, 16)
        self.mlp3 = MLP(in_features=384, hidden_features=4 * 384, drop=0)
        self.dt4 = testSelfAttention(512, 16)
...
class vit_snn(nn.Module):
    def __init__(self,
        ...
        patch_embed = testTokenizer(img_size_h=img_size_h,
        ...
        # classification head
        self.head = nn.Linear(ha_length, self.num_classes) if num_classes > 0 else nn.Identity()
        self.apply(self._init_weights)

        self.ha_layer = nn.Linear(embed_dims, ha_length)
        self.ha_lif = MultiStepSimLIFNode(tau=2.0, v_threshold=0.2, detach_reset=True, backend='cupy')
    ...
    def forward(self, x):
        ...
        ha_out1 = self.ha_layer(x)
        ha_out, sim_x = self.ha_lif(ha_out1)
        cls_out = self.head(ha_out.mean(0))
        ha_out = ha_out.prod(dim=0)
        mean_x = torch.mean(sim_x, dim=0)
        return mean_x, ha_out, cls_out


@register_model
def testformer(pretrained=False, **kwargs):
    model = vit_snn(
        **kwargs
    )
